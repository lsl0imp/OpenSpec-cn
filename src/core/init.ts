...
const toolSelectionWizard = createPrompt<string[], ToolWizardConfig>(
  (config, done) => {
    ...
    if (step === 'intro') {
      ...
      lines.push(
        PALETTE.midGray(
          '使用 ↑/↓ 移动 · Space 切换 · Enter 选择高亮的工具并查看。'
        )
      );
      lines.push('');
      lines.push(page);
...
export class InitCommand {
  ...
  private displaySuccessMessage(
    ...
    console.log(
      PALETTE.midGray(
        '使用 `openspec update` 在未来刷新共享的 OpenSpec 指令。'
      )
    );
    ...
    console.log(PALETTE.white('1. 填充项目上下文：'));
    console.log(
      PALETTE.lightGray(
    ...
    console.log(
      PALETTE.lightGray('    此功能的 OpenSpec 变更提案"\n')
    );
    console.log(PALETTE.white('3. 了解 OpenSpec 工作流程：'));
    console.log(
      PALETTE.lightGray(
        '   "请解释 openspec/AGENTS.md 中的 OpenSpec 工作流程'
      )
    );
    console.log(
      PALETTE.lightGray('    以及我应如何在此项目上与您合作"')
    );
    console.log(
      PALETTE.darkGray(
        '────────────────────────────────────────────────────────────\n'
